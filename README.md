...
준비물은 Node 24 이상, pnpm 11.17.0(루트 `package.json`의 `packageManager`로 고정), git(플러그인 마켓플레이스 추가가 이 저장소를 git clone으로 받아옵니다), 그리고 앱인토스 콘솔 계정입니다.
...
- `/ait:<verb>` 슬래시 명령 네임스페이스는 Codex에 그대로 오지 않습니다. Codex는 플러그인의 명령을 skill로 자동 변환하는데, 본문에서 `$ARGUMENTS` 치환을 쓰는 `new`는 이 변환에서 빠집니다. 다만 그 실체인 `new-miniapp` skill 자체는 설치되므로, 슬래시 명령 대신 자연어로 요청하면(예: "새 미니앱 my-app을 만들어 줘") 같은 절차를 탑니다. 나머지 skill 8종도 전부 skill로 직접 설치됩니다 — 명령 파일을 거치지 않습니다. 발화 예시는 아래 [말로 시키기](#말로-시키기--자연어-예시-5종)를 그대로 쓰면 됩니다 — 각 skill도 완료 안내에서 슬래시 명령과 자연어 동치를 함께 인쇄합니다.
...
문제를 발견하면 [버그리포트 가이드](./docs/bug-report-guide.md)를 먼저 참고한 뒤 [이슈를 등록](https://github.com/toss/apps-in-toss-harness/issues/new/choose)해 주세요. 이 저장소는 public이므로 Deploy Key·TOTP 등 시크릿이나 사내 식별자는 이슈 본문·로그에 붙여넣지 마세요.
