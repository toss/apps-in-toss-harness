...
`/ait:setup-debugger`는 on-device 디버깅(`/ait:debug` §5, 환경 2·3)이 쓰는
...
- 세션에서 서버를 승인·연결하면 `/mcp` 목록에 `ait-devtools`가 뜨고,
  `/ait:debug`의 `start_attach` QR attach 경로(환경 2·3)가 열린다.
...
연결되면 /ait:debug 가 start_attach QR attach 경로(환경 2·3)를 진행합니다.
```
...
배선이 끝나고 세션에서 서버 연결이 확인되면 바로 디버깅으로:

```
/ait:debug
```

`/ait:debug`가 bootstrap 도구(`start_attach` 등)를 확인하고 환경 2(relay-sandbox
QR)·환경 3(relay-staging QR) attach를 진행한다. 환경 2가 처음이면
`/ait:setup-phone-preview` 배선이 선행돼야 한다.
