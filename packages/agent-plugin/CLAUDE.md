...
`devtools` 단일 패키지에서 MCP 데몬·테스트 러너·on-device attach 표면이 `debugger` repo(`@apps-in-toss/debugger` + `@apps-in-toss/debug-console` 2개 패키지)로 분리됐다(Phase 3). `devtools`는 mock·panel·unplugin(브라우저 dev 필수품)만 남아 과도기 동안 devDep 전용으로 쓰이지만, 배포 모델은 wf(`@apps-in-toss/web-framework` 3.x)의 transitive dependency로 전환 진행 중이다(D1b) — 전환 후에는 소비자가 devtools를 직접 설치하지 않는다. 이 전환에서 platform으로 가는 건 **"SDK mock + panel + unplugin(mock 전달체)" 코어뿐**이다 — unplugin 안에서도 cloudflared quick tunnel·debug-console 자동 주입·launcher URL처럼 `debugger`/`debug-console`과 결합된 debug 표면은 이관 시 마저 절단되어 harness/`debugger` 쪽에 남는다(`packages/devtools/docs/porting-to-platform.md` 참고).
