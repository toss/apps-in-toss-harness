...
이 패키지는 `toss/apps-in-toss-harness` monorepo 소속 **토스 공식** 패키지다 — hardfork 완료(aitcc 트리밍, manifest 재작성, `/ait:<verb>` rename이 전부 이 repo에서 수행됨). 과거 `apps-in-toss-community/agent-plugin`의 커뮤니티 disclaimer("커뮤니티 오픈소스 프로젝트입니다." 등)는 넣지 않는다. 동시에 과장도 금지 — 상태는 정직하게 쓴다. **현재 상태: repo는 2026-08-26에 삭제 후 동일 이름으로 재생성됐고(이력 = 단일 `Initial commit`), 재생성 시점부터 이미 public이었다** — 실측(2026-08-27, REST `{"private":false,"visibility":"public"}`)으로 확인됐고 maintainer가 public 유지를 결정했다(2026-08-27). 패키지는 npm에 발행하지 않고 GitHub Releases로 유통한다 — 릴리즈 2건(`debugger-v0.2.0`·`debug-console-v0.1.4`)은 재생성 후 CI로 재발행됐고, 미인증 다운로드가 가능해 유통 경로가 열려 있다 — 이 패키지의 skill·문서에 박힌 Release URL이 정상 동작한다. 그 뒤 `debugger`는 v0.2.1(chii 프로세스 전역 TLS 검증 해제 부작용 방어 패치, 2026-08-27)로, `debug-console`은 v0.1.5(문서: 설치 안내 기본 패키지 매니저를 npm으로 전환)로 갱신 발행됐다 — 기존 발행분은 Immutable Releases로 잠김 유지. 자세한 원칙은 루트 `CLAUDE.md` "노출 산출물" 섹션.
...
| `design` | 디자인 station 8 — 화면 생성·개선·진단을 담당하는 생성형 skill. 3층 렌더 규칙(1층 하드=완료 차단·자동 수정, 2층 권장, 과잉지적 금지)과 기본 토큰은 `references/render-rules.md`(값의 정본은 `assets/project/tokens.css`), 요청 무게 분류·리스크 점검·화면 명세는 `references/build-mode.md`, 프로젝트 가이드 주입 절차는 `references/project-guide.md`, 사후 채점 기준은 `references/quality-bar.md`(G0~G8 항목별 등급). Figma MCP 연동 등록용 이미지 에셋 산출(콘솔 MCP `miniapp_create` 규격과 일치)도 계속 담당 | `/ait:design` | `Write`, `Edit`, Figma MCP, docs MCP |
...
| `test-on-device` | 실기기(토스 앱) 확인의 정규 경로 — `npm run build`(→`ait build`) → 콘솔 MCP `bundle_upload`/`bundle_upload_complete` → `miniapp_get_status`로 컴파일 확인 → 도구가 돌려준 링크로 토스 앱에서 열기. appName 일치 가드 포함. 검수 제출·릴리즈/롤백·프로모션은 scope 밖 | `/ait:test-on-device` | `Bash`, 콘솔 MCP |
