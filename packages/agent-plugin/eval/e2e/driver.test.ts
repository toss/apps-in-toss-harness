...
describe('isForbiddenBashCommand', () => {
  ...
  const FORBIDDEN = [
    ...
    // create-ait-app 산출물 package.json의 "deploy": "ait deploy" 스크립트를
  ...
  const ALLOWED = [
    ...
    // 버전은 명시 핀 없이 @latest(maintainer 결정 2026-08-10), --template과
    // --tds는 동시 지정이 금지(택일)이므로 한쪽만 쓴다.
    'pnpm dlx create-ait-app@latest coupon-shop --inline --pm pnpm --template react-ts',
    'pnpm dlx create-ait-app@latest coupon-shop --inline --pm pnpm --tds',
    // "deploy-" 로 시작하는 다른 이름의 스크립트/앱명 오탐 가드 (`deploy` 뒤
    // 부정 lookahead).
    'pnpm dlx create-ait-app@latest deploy-demo --inline --pm pnpm --template react-ts',
    'pnpm --dir ./timer add -D @apps-in-toss/devtools', // Step 4 폴백(CLI가 배선하지 않았을 때의 수동 배선)
    'pnpm --dir ./timer remove @apps-in-toss/devtools', // Step 4-b 배선 해제(--no-devtools)
...
// 콘솔 MCP 서버(apps-in-toss-console) 소속 도구 판정. plugin manifest는 이 서버를
// 기본 포함하지만, driver.ts가 options.plugins/options.mcpServers를 넘기지 않는
// 한 이 harness 세션엔 그 서버가 로드되지 않는다(README.md "세션 형상" 절 참고) —
// 그래도 세션 구성이 나중에 바뀌었을 때를 대비한 2차 방어 회귀 가드다. 직접 등록
// 형태(mcp__apps-in-toss-console__…)와 플러그인 경유 형태
// (mcp__plugin_<pluginName>_apps-in-toss-console__…) 둘 다 잡아야 한다 — 도구
// 이름이 늘어나도(miniapp_create 외에 신규 도구가 추가돼도) 서버 키 prefix
// 판정이라 그대로 유효해야 한다.
describe('isConsoleMcpTool', () => {
  const CONSOLE_TOOLS = [
    ...
    // 플러그인 경유 형태 (`mcp__plugin_<pluginName>_<serverKey>__<tool>` — 실측 예:
    // `mcp__plugin_playwright_playwright__browser_click`,
    // `mcp__plugin_slack_slack__slack_send_message_draft`). 이 harness plugin의
    // manifest 등록 이름은 `ait`(`.claude-plugin/plugin.json`).
    'mcp__plugin_ait_apps-in-toss-console__miniapp_create',
    'mcp__plugin_ait_apps-in-toss-console__bundle_upload',
    // 마켓플레이스 설치명이 다른 이름으로 바뀌어도(충돌 회피 등) 서버 키
    // 세그먼트만 보고 잡아야 한다 — plugin 세그먼트를 고정하지 않는 이유.
    'mcp__plugin_apps-in-toss-harness_apps-in-toss-console__miniapp_get_status',
  ];
  ...
  const OTHER_TOOLS = [
    ...
    'mcp__plugin_ait_apps-in-toss-docs__searchDocumentation', // docs MCP 플러그인 경유 형태 — 콘솔 아님
    'mcp__plugin_ait_ait-devtools__start_attach', // ait-devtools 플러그인 경유 형태 — 콘솔 아님
    'Bash',
    'Read',
    '',
  ];
  ...
  it('플러그인 경유 형태도 서버 키 부분 문자열만으론 오탐하지 않는다', () => {
    expect(isConsoleMcpTool('mcp__plugin_ait_apps-in-toss-console-v2__miniapp_create')).toBe(false);
    // `plugin_` 세그먼트가 실제로 없는 경우는 이 정규식이 아니라 직접 등록 형태
    // 판정(위)이 담당 대상이다 — 이 문자열은 둘 중 어느 쪽에도 안 걸려야 한다.
    expect(isConsoleMcpTool('mcp__ait_apps-in-toss-console__miniapp_create')).toBe(false);
  });
});
...
/**
 * disallowedTools 항목에서 서버 키만 뽑는다. 직접 등록 형태(`mcp__<serverKey>`)와
 * 플러그인 경유 형태(`mcp__plugin_<pluginName>_<serverKey>`) 둘 다 지원한다 —
 * STATIC_DISALLOWED_TOOLS가 이제 두 형태를 나란히 두므로(driver.ts), "실재하는
 * 서버 키를 가리키는가" 검사가 플러그인 경유 항목에서 공허하게 통과하지 않으려면
 * 이 추출이 두 형태를 모두 이해해야 한다.
 */
function bareServerKey(disallowedEntry: string): string {
  const withoutMcpPrefix = disallowedEntry.replace(/^mcp__/, '');
  const pluginRouted = withoutMcpPrefix.match(/^plugin_.+?_(.+)$/);
  return pluginRouted ? pluginRouted[1] : withoutMcpPrefix;
}

describe('bareServerKey', () => {
  it('직접 등록 형태에서 서버 키를 그대로 뽑는다', () => {
    expect(bareServerKey('mcp__apps-in-toss-console')).toBe('apps-in-toss-console');
    expect(bareServerKey('mcp__ait-devtools')).toBe('ait-devtools');
  });

  it('플러그인 경유 형태에서 plugin 세그먼트를 벗기고 서버 키만 뽑는다', () => {
    expect(bareServerKey('mcp__plugin_ait_apps-in-toss-console')).toBe('apps-in-toss-console');
    expect(bareServerKey('mcp__plugin_ait_ait-devtools')).toBe('ait-devtools');
  });
});
...
describe('MCP 서버 키 ↔ disallowedTools 결합', () => {
  ...
  it('직접 등록 형태로 차단되는 모든 서버 키는 플러그인 경유 형태로도 나란히 차단돼야 한다', () => {
    // STATIC_DISALLOWED_TOOLS에 `mcp__<key>`가 있으면 `mcp__plugin_<any>_<key>`류
    // 짝도 있어야 한다 — 한쪽만 개명/삭제되는 조용한 드리프트를 잡는다.
    const directKeys = STATIC_DISALLOWED_TOOLS.filter((t) => !t.startsWith('mcp__plugin_')).map(
      (t) => bareServerKey(t),
    );
    const pluginRoutedKeys = new Set(
      STATIC_DISALLOWED_TOOLS.filter((t) => t.startsWith('mcp__plugin_')).map((t) =>
        bareServerKey(t),
      ),
    );
    for (const key of directKeys) {
      expect(
        pluginRoutedKeys.has(key),
        `\`mcp__${key}\`는 직접 등록 형태로만 차단 목록에 있고 플러그인 경유 짝이 없다.`,
      ).toBe(true);
    }
  });

  it('차단 대상 manifest 서버는 isConsoleMcpTool prefix 판정에도 걸려야 한다 (직접 등록·플러그인 경유 둘 다)', () => {
    ...
    for (const key of blockedManifestKeys) {
      ...
      expect(
        isConsoleMcpTool(`mcp__plugin_ait_${key}__some_tool`),
        `prefix 판정이 플러그인 경유 형태의 \`${key}\` 를 놓친다`,
      ).toBe(true);
    }
  });
});
