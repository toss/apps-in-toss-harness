...
#      들어가고 **command stub 4개가 같은 목록에 함께 오른다**. 이 차이는
...
#
# 남은 stub 4개는 전부 이름이 대응 skill과 다르다(harness#134 — 겹치던 6개는
# 삭제됐다. 겹치면 command가 이겨 skill 본문이 아예 안 뜬다). 그중
# 3개(inject-devtools/inject-debug-console/inject-tossface)는 facet 병합이라
# `inject`로 펴야 하고(§제공물), 안 펴면 `inject-tossface`만 부른 run이
# "inject 안 뜸"으로 오판된다. `ait-*)` 케이스는 stub 파일명이 전부 bare verb로
# 개명된 뒤로는 관측되지 않지만(#286 이후), 구형 플러그인 캐시 등 방어적으로
# 남긴다.
_canon() {
  case "$1" in
    ait-new|new)                        echo new-miniapp ;;
    ait-inject-devtools|ait-inject-debug-console|ait-inject-tossface) echo inject ;;
    inject-devtools|inject-debug-console|inject-tossface) echo inject ;;
    ait-*)                              echo "${1#ait-}" ;;
    *)                                  echo "$1" ;;
  esac
}
