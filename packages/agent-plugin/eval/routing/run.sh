...
#      들어가고 **command stub 9개가 같은 목록에 함께 오른다**. 이 차이는
...
# 이름으로 관측되므로 이 케이스는 애초에 정규화가 필요 없다). stub 9개 중
# 2개(inject-devtools/inject-debug-console)만 이름이 skill과
# 다르다(§제공물의 facet 병합) — 그걸 안 펴면 `inject-debug-console`만 부른 run이
...
_canon() {
  case "$1" in
    ...
    ait-inject-devtools|ait-inject-debug-console) echo inject ;;
    inject-devtools|inject-debug-console) echo inject ;;
    ait-*)                              echo "${1#ait-}" ;;
    *)                                  echo "$1" ;;
  esac
}
