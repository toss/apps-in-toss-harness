 ...
 * `@apps-in-toss/devtools/in-app` graph. A consumer wraps its
...
export async function mountEruda(): Promise<void> {
  ...
  try {
    ...
    erudaMounted = false;
    console.debug('[@apps-in-toss/devtools] eruda console mount skipped:', err);
  }
}
...
export function unmountEruda(): void {
  ...
  try {
    ...
    console.debug('[@apps-in-toss/devtools] eruda console unmount skipped:', err);
  } finally {
