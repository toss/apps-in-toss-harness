 ...
 *
 * One chrome variant is precompiled per locale, carrying the env-3 (relay-dev)
 * Toss app deep-link flow. The `sandbox` family (#468) that carried the env-2
 * launcher PWA flow was removed with the PWA Sandbox environment (harness#103).
 *
 * Token-fill vs runtime assembly:
 *   - `attachChromeByLocale[locale]` (exported from the generated module): full
 *     static HTML rendered at build time. Contains NO per-request values.
...
interface AttachHtmlProps {
  ...
  strings: {
    title: string;
    /** `deployment: ` label prefix for the `_deploymentId` row. */
    deploymentPrefix: string;
    stepsSection: string;
    /**
     * Scan steps, in order. May contain inline HTML (strong/code) — trusted
     * build-time copy, never user input.
     */
    steps: string[];
    faqSection: string;
    /**
     * Troubleshooting checklist items, in order. May contain inline HTML
