/**
 * Deep-link helpers for the intoss-private (env 3) attach path.
 *
 * The env-2 launcher PWA helpers (`LAUNCHER_URL` / `resolveLauncherUrl` /
 * `buildLauncherAttachUrl` / `buildLauncherDeepLink`) lived here until the PWA
 * Sandbox environment was removed in full (harness#103, 2026-08-10 maintainer
 * decision). What remains is the intoss-private scheme-URL splicer used by
 * `start_attach` in the env-3 dog-food path.
 */
