 ...
 *       "ait-devtools": {
 *         "command": "npx",
 *         "args": ["-y", "-p", "https://github.com/toss/apps-in-toss-harness/releases/download/debugger-v0.2.2/apps-in-toss-debugger-0.2.2.tgz", "debugger", "--mode=dev"],
...
const DEV_TOOL_DEFINITIONS = [
  ...
  /* ------------------------------------------------------------------ */
  {
    ...
    description:
      ...
      'To use this tool: restart the MCP server with `--mode=debug` (or omit --mode), then call ' +
      'start_attach({mode:"relay-staging", …}) to bring up the relay and generate the QR for ' +
      'phone scanning. No environment variable is involved. ' +
...
export function createDevServer(deps: CreateDevServerDeps = {}): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (TIER_B_TOOL_NAMES.has(name)) {
      // dev-mode는 debug 데몬과 별개 프로세스라 런타임 warm swap 대상이 아니다 —
      // 여기서는 재시작이 실제로 필요하므로 기본 hint를 override한다.
      // 런타임 표면(tool description·에러 hint)은 **할 일만** 말한다. 죽은
      // `MCP_ENV`를 여기서 이름으로 부르면, 그 변수를 모르던 에이전트에게 오히려
      // 존재를 알려 잘못된 경로를 만든다 — "재시작 뒤 start_attach"라는 양성
      // 경로만 준다. 죽었다는 사실 자체의 정본은 debug skill의
      // `references/mode-switching.md`다.
      return tierRejectionError(
        name,
        'relay',
        'mock',
        'dev-mode — Vite HTTP endpoint, no CDP/relay connection',
        'dev-mode 서버에는 CDP·relay 연결이 없습니다. `--mode=debug`로(또는 `--mode` 없이) ' +
          '데몬을 재시작한 뒤 `start_attach({mode:"relay-staging", scheme_url, projectRoot})`로 ' +
          '실기기를 attach하세요. 환경 변수를 설정할 필요는 없습니다.',
      );
    }
