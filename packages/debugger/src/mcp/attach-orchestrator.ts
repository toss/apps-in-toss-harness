 ...
 * (issue #610).
 *
 * Rationale: when a relay goes offline, the daemon's warm `ChiiCdpConnection`
 * still lists the last-seen target — its `lastSeenAt` freezes at the moment the
 * old relay died. A 5-minute threshold is large enough to be invisible in
 * normal usage (active CDP sessions see a message every few seconds) while
 * being small enough to catch a relay that went down before the daemon was
 * re-entered.
 *
 * Injectable for tests via `DebugServerDeps.stalePageThresholdMs`.
 */
export const RELAY_STALE_PAGE_MS = 5 * 60 * 1_000; // 5 minutes
...
/**
 * Predicate used by `run_tests` (and the attach wait loop) to decide whether a
 * relay connection has a genuinely fresh page attached.
 *
 * Stale-ghost gating (issue #610): when a relay is replaced, the warm
 * `ChiiCdpConnection` still lists the last-seen target but its `lastSeenAt` is
 * frozen. A page whose `lastSeenAt` exceeds
 ...
 */
export function isRelayPageFresh(
 ...
 *
 * `kind: 'scheme'` = env 3 (intoss-private deep-link, `buildDeepLinkAttachUrl`).
 * The env-2 `kind: 'launcher'` variant was removed with the PWA Sandbox
 * environment (harness#103).
 *
 * SECRET-HANDLING: these components contain scheme hosts. They are
 ...
 */
export type AttachUrlParts = { kind: 'scheme'; schemeUrl: string; wssUrl: string };
...
/**
 * Output of the {@link prepareAttach} helper (issue #626) — the validation +
 * component bundle the env-3 (relay-dev) attach path produces. On any
 * validation failure the helper returns `{ ok: false, error }` with a
 * ready-to-return `McpResult`.
 */
...
/**
 * Validation + component bundle for `start_attach` (issue #626). Requires
 * `scheme_url` (env 3 / relay-dev intoss-private deep-link) and builds the
 * scheme parts. Returns `{ ok: false, error }` with a ready McpResult on any
 * failure.
 *
 * The env-2 (`relay-mobile`) launcher-PWA branch was removed with the PWA
 * Sandbox environment (harness#103).
 */
...
export async function renderAndMaybeWait(
  ...
  const warningPrefix = authorityNotice;
 ...
 *      present (mock env / no bridge) the pending/last lines are simply empty
