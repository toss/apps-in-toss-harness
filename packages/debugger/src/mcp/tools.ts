...
export const DEBUG_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      ...
      'mode (optional): pass "relay-staging" (env 3) to switch the active environment first. ' +
      'When omitted, the current relay environment is used as-is (no switch). ' +
      'Passing "local-browser" returns an error — start_attach is relay-only (env 3). ' +
      'When the session is already in the requested mode, the switch is skipped.\n\n' +
      'Requires scheme_url — the intoss-private://…?_deploymentId=<uuid> URL from ' +
      '`ait deploy --scheme-only`. Splices debug=1 + relay URL into the scheme URL.\n\n' +
    ...
    inputSchema: {
      ...
      properties: {
        mode: {
          type: 'string',
          enum: ['local-browser', 'relay-staging'],
          description:
            'Optional debug mode to switch into before attaching. "relay-staging" = env 3 ' +
            '(intoss-private dog-food). "local-browser" returns an error (start_attach is ' +
            'relay-only). Omit to keep the current relay environment.',
        },
        scheme_url: {
          type: 'string',
          description:
            'The intoss-private:// scheme URL from `ait deploy --scheme-only` (must carry _deploymentId). ' +
            'Required — start_attach has no other attach path. ' +
        ...
        projectRoot: {
          type: 'string',
          description:
            'Absolute path to the mini-app project root (the directory containing its package.json). ' +
            'When switching into a relay mode, the daemon reads the relay TOTP secret read-only from ' +
            "<projectRoot>/.ait_relay. Pass this because the daemon's own cwd is fixed at launch.",
        },
      },
  ...
  },
  {
    ...
    description:
      ...
      'The result includes a `source: "mock" | "relay-dev"` field so consumers can identify ' +
      'provenance without inspecting payload values. ' +
      '(`relay-dev` = env 3 dog-food WebView; relay-live/env 4 removed #665, ' +
      'relay-mobile/env 2 removed #103.) ' +
  ...
  },
  {
    ...
    description:
      ...
      'On env 3 (real device relay) this hits the real SDK; on env 1 (local mock) ' +
      'it hits the mock SDK. ' +
  ...
  },
  {
    ...
    description:
      ...
      'relay (env 3, real-device over the Chii relay + cloudflared tunnel) ' +
      ...
      '*.private-apps.tossmini.com and the cloudflared quick-tunnel host that fronts it (env 3) ' +
      'are allowed.\n\n' +
      'modes:\n' +
    ...
    inputSchema: {
      ...
      properties: {
        mode: {
          ...
          enum: ['local-browser', 'relay-staging'],
          description:
            'Target environment to switch to. relay-live (env 4) was removed in #665 and relay-sandbox (env 2) in #103 — use relay-staging (env 3) for dog-food debugging.',
        },
        projectRoot: {
          type: 'string',
          description:
            'Absolute path to the mini-app project root (the directory containing its package.json and .ait_relay). ' +
            'The daemon reads the relay auth secret from <projectRoot>/.ait_relay (read-only) when switching to the relay ' +
            "environment (relay-staging). Pass this because the daemon's own cwd is fixed at launch and may not be " +
  ...
  },
  {
    ...
    description:
      ...
      'environment (kind: mock|relay-dev, env: mock|relay backward-compat, reason, ' +
      'liveGuardActive: always false — relay-live and LIVE guard removed (#665); ' +
      'start_debug mode→kind mapping: relay-staging→relay-dev, local-browser→mock), ' +
 ...
 *
 * The relay env (`relay-dev`) satisfies the `'relay'` availability tier —
 * `isRelayEnv()` is used for the check. (`relay-live` removed #665,
 * `relay-mobile` #103.)
 */
 ...
 *
 * The relay env (`relay-dev`) satisfies the `'relay'` tier.
 * (`relay-live` removed #665, `relay-mobile` #103.)
 */
...
export interface SafeAreaMeasurement {
   ...
   *   (`relay-live` / env 4 removed #665, `relay-mobile` / env 2 #103.)
   *
 ...
 *
 * On env 3 (toss WebView relay) this hits the real SDK. On env 1 (local mock)
 * it hits the mock SDK.
 *
...
export interface DiagnosticsResult {
   ...
   *
   * `kind` — the precise environment (`mock` | `relay-dev`). Use this for new
   *   code. (`relay-live` removed #665, `relay-mobile` #103.)
...
export interface GetDiagnosticsInput {
  ...
  connection?: CdpConnection;
  /**
   * Resolved MCP environment (`mock` | `relay-dev`).
   * Caller obtains via `resolveEnvironment()`.
   * (`relay-live` removed #665, `relay-mobile` #103.)
   */
  env: McpEnvironment;
