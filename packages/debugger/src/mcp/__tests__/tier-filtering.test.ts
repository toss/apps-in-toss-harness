...
describe('tool-registry — pure helpers', () => {
  ...
  it('isToolAvailableIn respects the env decision (relay-live #665 / relay-mobile #103 removed)', () => {
    // Tier B → relay only.
  ...
  // relay-live (#665) and relay-mobile (#103) removed — no
  // filterToolsByEnvironment tests for them.
});
...
describe('tools/list — env filtering integration', () => {
  ...
  // relay-live (#665) and relay-mobile (#103) removed — no tools/list tests
  // for them.
...
describe('tools/list — env derived from connection.kind (issue #348)', () => {
  ...
  async function makeRealEnvClient(opts: {
    ...
    const server = createDebugServer({
      ...
      // the real `deriveEnvironment(connection.kind)` path.
    });
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    await server.connect(serverTransport);
...
describe('tools/call — env mismatch rejection', () => {
  ...
  it('relay-dev passes side-effect tools without any confirm gate (#665)', async () => {
    // relay-dev is a dev-intent env — the LIVE guard is removed (#665).
    // evaluate/call_sdk must NOT require confirm: true on any relay env. Use a
    // private-apps dog-food URL — the positive-allowlist allows it.
    const targets: CdpTarget[] = [
      { id: 't1', title: 'app', url: 'https://app.private-apps.tossmini.com/app' },
    ];
    const connection = new FakeCdpConnection(targets);
    const server = createDebugServer({
      connection,
