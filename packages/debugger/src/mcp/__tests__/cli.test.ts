...
describe('parseMode', () => {
  ...
  it('unknown-mode error message lists both modes', () => {
    expect(() => parseMode(['--mode=bogus'])).toThrow(/'debug' \(default\) or 'dev'/);
  });
  ...
  it('does not parse --mode after a bare -- boundary', () => {
    expect(parseMode(['--', '--mode=dev'])).toBe('debug');
  });
});
...
describe('findUnknownFlags', () => {
  ...
  it('does not flag tokens after a bare -- boundary', () => {
    expect(findUnknownFlags(['--mode=dev', '--', 'vite', '--host', '--bogus-vite-flag'])).toEqual(
      [],
    );
  });
