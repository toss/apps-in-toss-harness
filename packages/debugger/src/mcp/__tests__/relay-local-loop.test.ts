/**
 * relay 관측 루프 로컬 PC 검증 — full attach → list_pages → measure_safe_area
 *
 * WHAT THIS TEST PROVES:
 *   relay MCP-attach 관측 루프(attach → list_pages → measure_safe_area) 전체가
 ...
 *   실기기 엔진이 생성하는 실제 safe-area 수치(`env(safe-area-inset-*)`)는 이
 *   테스트가 확인하지 않는다 — 그건 실기기에서만 얻을 수 있는 device-engine
 *   fidelity이지 루프 자체의 검증 갭이 아니다. attach/observe 루프는 완전히
 *   로컬 PC에서 검증된다.
 *
 * NO PRODUCTION CODE CHANGES:
 *   이 테스트는 gate/attach 등 프로덕션 코드를 수정하지 않는다. 모든 검증은 노드 측
 ...
 *
 * Shape mirrors a real device response:
...
const STUB_SAFE_AREA_JSON = JSON.stringify({
  ...
  innerHeight: 844,
  devicePixelRatio: 3,
  userAgent: 'relay-local-loop-stub/1.0',
});
...
/**
 * Fake Chii relay for the local relay-loop test.
 *
...
describe('relay local-PC full loop — attach → list_pages → measure_safe_area', () => {
  it('full chain: ChiiCdpConnection attaches, lists target, and measureSafeArea returns SafeAreaMeasurement', async () => {
    ...
    try {
      ...
      expect(beforeEnable[0]?.id).toBe('relay-target-1');
      ...
      expect(targets[0]?.id).toBe('relay-target-1');
      expect(targets[0]?.title).toBe('Relay Stub Mini-App');
      ...
      const measurement = await measureSafeArea(conn, 'relay-dev');

      // Assert the full SafeAreaMeasurement shape — this is exactly what the
      // production relay observe loop produces (minus real device numbers).
      expect(measurement.source).toBe('relay-dev');

      // cssEnv: the stub reflects iPhone-style insets.
