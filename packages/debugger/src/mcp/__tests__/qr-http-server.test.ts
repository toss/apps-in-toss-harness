...
//
// DashboardState.mode에 따라 attach 페이지 상단 환경 라벨이 달라진다:
//   - relay-dev (환경 3) → intoss 카피 + "환경 3" 라벨
//   - mode 미설정/mock   → 같은 카피, 환경 라벨 없음 (기존 동작 보존)
//   - relay-live (환경 4) 제거 (#665) — positive-allowlist kill-switch.
//   - relay-mobile (환경 2) 제거 (#103) — sandbox 카피 family도 함께 제거.
//
// SECRET-HANDLING: placeholder URL만 사용 — 실제 터널/relay/TOTP 값 없음.
// ---------------------------------------------------------------------------

describe('startQrHttpServer — /attach mode-aware chrome 분기 (#468)', () => {
  // 환경 3: intoss-private deep-link attach URL
