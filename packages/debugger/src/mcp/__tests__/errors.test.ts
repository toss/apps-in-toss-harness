...
describe('tierRejectionError — Tier A/B 환경 불일치', () => {
  ...
  // 죽은 환경 변수 안내 회귀 방지 (#138 defect 7).
  //
  // `MCP_ENV`는 소스 어디에서도 읽지 않는다(#665로 env 파생에서 제거됨 —
  // `cli.ts` 헤더 주석, `tier-filtering.test.ts`의 ambient scrub 주석). 그런데
  // 종전 hint는 "MCP_ENV=relay 설정 후 서버를 재시작하세요"라고 안내했다.
  // 그 지시를 그대로 따르면 재시작 후에도 환경이 그대로라 같은 거부를 다시
  // 받는다 — 에이전트가 빠져나올 수 없는 루프가 된다. hint는 반드시 실제로
  // 동작하는 런타임 전환 도구를 가리켜야 한다.
  it('hint가 죽은 MCP_ENV 변수를 안내하지 않는다 — 양방향', () => {
    const toRelay = getText(tierRejectionError('start_attach', 'relay', 'mock', 'default-mock'));
    const toMock = getText(
      tierRejectionError('get_mock_state', 'mock', 'relay', 'derived:kind=relay'),
    );
    expect(toRelay).not.toContain('MCP_ENV');
    expect(toMock).not.toContain('MCP_ENV');
  });

  it('hint가 재시작 없는 런타임 전환 도구를 가리킨다', () => {
    const toRelay = getText(tierRejectionError('call_sdk', 'relay', 'mock', 'default-mock'));
    expect(toRelay).toContain('start_attach');
    expect(toRelay).toContain('relay-staging');

    const toMock = getText(
      tierRejectionError('get_mock_state', 'mock', 'relay', 'derived:kind=relay'),
    );
    expect(toMock).toContain('start_debug');
    expect(toMock).toContain('local-browser');
  });

  it('hint override — 런타임 swap이 불가능한 호출자(dev-mode)는 자기 안내를 넘긴다', () => {
    const text = getText(
      tierRejectionError('start_attach', 'relay', 'mock', 'dev-mode', '커스텀 복구 안내'),
    );
    expect(text).toContain('커스텀 복구 안내');
    // override가 기본 hint를 대체하지, 덧붙이지 않는다.
    expect(text).not.toContain('서버 재시작은 필요 없습니다');
  });
});
