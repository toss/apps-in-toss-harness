...
export const RELAY_AUTH_SECRET_MISSING_MESSAGE = [
  ...
  '.ait_relay는 @apps-in-toss/devtools unplugin이 dev 서버 첫 기동 때 만들어 둡니다 — 이미 있다면 projectRoot만 전달하면 됩니다.',
  '없다면 위 명령으로 발급한 값을 AIT_DEBUG_TOTP_SECRET 환경변수에 직접 설정하세요.',
 ...
 * comes up unauthenticated, so this guard is called at the relay-boot site
 * (`bootRelayFamily`, the intoss relay). Local-only sessions never boot a relay
 * and so never reach this guard, matching the issue's exemption for non-relay
 * debugging.
 *
 ...
 * — the daemon refuses to come up rather than booting an unauthenticated relay.
 *
 ...
 *     phone → camera scan → Toss app cold-loads the bundle → attach) routinely
 *     exceeds the 90 s window of the RFC default (skew=1), especially when the
 *     phone is not immediately at hand.
 ...
 * minutes, which is enough to cover the QR-scan → attach flow even when the
 * Toss app has to cold-load the bundle first (#490).
 *
 ...
 *
 * Lives here (not in the MCP server) so the unplugin's relay can wire the same
 * gate without importing the heavy MCP server module graph. Re-exported from
 * `debug-server.ts` for back-compat.
 *
