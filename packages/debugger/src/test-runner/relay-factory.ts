 ...
 *
 *   import { createRelayConnectionFactory } from '@apps-in-toss/debugger/test-runner';
 ...
 * `@apps-in-toss/debugger/test-runner` barrel) does NOT statically drag that graph in.
 *
...
// `@apps-in-toss/debugger/test-runner` barrel (config.ts) does NOT statically drag the
...
export interface RelayConnectionFactoryOptions {
   ...
   * the `@apps-in-toss/debugger/test-runner` barrel is forced to make an explicit
 ...
 * connection for env 3 (intoss-private scheme deep-link). The 환경 2 launcher
 * deep-link mode (`attachLauncher`, devtools#774) went away with the PWA
    ...
    async open(): Promise<CdpConnection> {
      ...
      try {
        ...
        const getDashboardState = (): DashboardState => ({
          ...
          // Label the dashboard chrome for the env being attached. Mirrors the
          // MCP daemon's mode selection.
