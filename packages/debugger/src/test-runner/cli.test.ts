 ...
 * flows into report/capture filenames). The `ios-pwa` value was the 환경 2
 * axis; it went away with the environment itself (harness#103).
 */
describe('resolveCellPlatform (devtools#774)', () => {
  ...
  it('rejects the removed 환경 2 value "ios-pwa" (#103)', () => {
    expect(resolveCellPlatform('ios-pwa').ok).toBe(false);
  });
...
describe('debugger-test main() — --cell-platform validation (devtools#774)', () => {
  ...
  it('accepts --cell-platform ios and injects it as the cell platform', async () => {
    await main([...ARGS, '--cell-platform', 'ios']);
    expect(process.exitCode).not.toBe(1);
    const factoryOpts = createRelayConnectionFactoryMock.mock.calls[0]?.[0] as {
      cell?: { platform?: string };
    };
    expect(factoryOpts?.cell?.platform).toBe('ios');
  });
});
