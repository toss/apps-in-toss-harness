/**
 * `debugger-test` bin entry point — export-free by design.
 *
 ...
 * every `debugger-test` / `pnpm test:env3` invocation exited 0 as a silent
 * no-op.
 *
...
main().catch((e: unknown) => {
  process.stderr.write(
    `debugger-test: unexpected error: ${e instanceof Error ? e.message : String(e)}\n`,
  );
  process.exitCode = 1;
});
