...
// ── attach env is always relay-dev (#103) ───────────────────────────────────
//
// 환경 2(launcher PWA) 제거 후 relay 팩토리의 attach 경로는 하나뿐이다:
// scheme deep-link + `relay-dev`. 별도 분기가 다시 생기지 않았는지 고정한다.
//
// SECRET-HANDLING: scheme URL은 합성 placeholder — 실제 호스트 없음.

describe('createRelayConnectionFactory — attach env (#103)', () => {
  const onQrContent = vi.fn();
  ...
  it('prepareAttach는 항상 relay-dev + scheme_url로 호출된다', async () => {
    const factory = createRelayConnectionFactory({
      schemeUrl: SYNTHETIC_SCHEME_URL,
      onQrContent,
    });
    ...
    // prepareAttach(deps, env, args, conn) — 2번째 positional이 env다.
