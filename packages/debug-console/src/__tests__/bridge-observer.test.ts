...
beforeEach(async () => {
  vi.resetModules();
  ({ installBridgeObserver, uninstallBridgeObserver } = await import('../bridge-observer.js'));
});
...
afterEach(() => {
  ...
  const win = w();
  // `delete` rather than `= undefined`: these are optional properties, so
  // planting an own property whose value is `undefined` is not the same as
  // removing it — the observer's install path feature-detects with `typeof`,
  // and the workspace compiles with `exactOptionalPropertyTypes`.
  delete win.__appsInTossNativeBridge;
  delete win.ReactNativeWebView;
  delete win.__GRANITE_NATIVE_EMITTER;
  delete win.__ait_bridge;
});
