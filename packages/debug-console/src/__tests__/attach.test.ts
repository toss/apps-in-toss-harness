...
describe('maybeAttach', () => {
  ...
  // ---------------------------------------------------------------------------
  // Auth-block signal (#478 defect 2)
  //
  // 환경 2(launcher PWA) 제거 전에는 부모 셸로 `ait:debug-attach-blocked`
  // postMessage를 보냈다. 환경 3에서는 미니앱이 top-level 문서라 받을 셸이
  // 없으므로 페이지 콘솔(warn)에 남긴다 — 데몬이 `list_console_messages`로
  // 읽어가는 경로다 (harness#103).
  // ---------------------------------------------------------------------------

  it('auth block — warns exactly once with a secret-free message', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

    maybeAttach(blockResult('auth'));

    expect(warnSpy).toHaveBeenCalledTimes(1);
    expect(String(warnSpy.mock.calls[0]?.[0])).toMatch(/relay rejected the TOTP code/);
    // SECRET-HANDLING: no code, secret, host, or relay URL in the message.
    expect(String(warnSpy.mock.calls[0]?.[0])).not.toMatch(/wss?:\/\/|\d{6}/);
  ...
  it.each([
    ...
    'host',
    'entry',
  ] as const)('%s block — does NOT warn (false-positive guard on ordinary pageviews)', (reason) => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

    maybeAttach(blockResult(reason));
  ...
  it('auth block — never posts to window.parent (launcher shell path is gone)', () => {
...
describe('installRelayWsObserver', () => {
  ...
  beforeEach(async () => {
    ...
    // The 4401 signal lands in the page console (harness#103) — capture it.
  ...
  it('warns auth-expired ONCE when a relay-origin socket closes 4401', () => {
    ...
    expect(message).toMatch(/relay rejected the TOTP code/);
    // SECRET-HANDLING: the notice names neither the code nor the relay URL.
...
describe('installRelayWsObserver — ait:relay-ws-state broadcast (#730)', () => {
  ...
  it('regression: the 4401 auth-expired console notice is unaffected by the new broadcast', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
...
describe('script.onerror fetch probe', () => {
  ...
  it('warns auth-expired when the probe of the script URL returns 401', async () => {
