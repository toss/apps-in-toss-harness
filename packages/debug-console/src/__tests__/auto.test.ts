/**
 * Unit tests for the `@apps-in-toss/debug-console/auto` self-gating side-effect
...
import { detectDevSignal, shouldActivate } from '../auto.js';
...
describe('maybeAttach() — called when gate activates', () => {
  ...
  it('calls maybeAttach() on import (DEV=true in vitest)', async () => {
    ...
    vi.clearAllMocks();
    await import('../auto.js');
    const attachMod = (await import('../attach.js')) as unknown as {
