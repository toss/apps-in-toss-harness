...
describe('mountEruda', () => {
  it('calls eruda.init() once on success', async () => {
    const { mountEruda } = await import('../eruda-overlay.js');
    await mountEruda();
...
describe('unmountEruda (#748 graceful detach)', () => {
  it('calls eruda.destroy() when eruda was mounted', async () => {
    const { mountEruda, unmountEruda } = await import('../eruda-overlay.js');
    await mountEruda();
    expect(initSpy).toHaveBeenCalledTimes(1);
  ...
  it('is a no-op when eruda was never mounted', async () => {
    const { unmountEruda } = await import('../eruda-overlay.js');
    unmountEruda();
