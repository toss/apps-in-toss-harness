...
| `scope-install` | **off** (`NORMALIZE_SCOPE_INSTALL=1`) | 설치 명령(`npm install`/`npx`/`pnpm add`/`yarn add`/`bun add`), npm 레지스트리 URL(`npmjs.com/package/@ait-co/...`), 설치 감지용 `grep` 문자열. 4개 대상 패키지(`devtools`/`debugger`/`debug-console`/`internal-protocol`)를 하나의 스코프 정규식으로 묶어 처리하지만, **해제 조건은 패키지마다 다르다** — 한 문장으로 뭉뚱그리면 안 된다. `devtools`는 wf 소스 monorepo(사내)가 발행 주체가 되어 **2026-08-04에 `@apps-in-toss/devtools@3.0.2`로 공개 npm(registry.npmjs.org)에 실제 배포됐다** — 이 패키지에 한해서는 해제 조건(실 npm 배포)이 이미 충족됐고, 스코프 install 리네임을 켜도 더 이상 404가 아니다(별도 축으로 진행 중, harness#74). 반면 `debugger`/`debug-console`은 npm-less 전환(GitHub Releases tarball 유통)이 **설계 의도**라 npmjs에는 앞으로도 발행되지 않는다 — 이 둘에 대해서는 "아직 배포 안 됨"이 아니라 "영구 미발행"이므로 해제 조건 자체가 다르게 정의돼야 하고, 스코프 install 리네임을 켜면 존재하지 않는 `npm install @apps-in-toss/debugger`류 명령을 안내하게 되는 회귀가 된다. `internal-protocol`은 `private: true`라 애초에 배포 대상이 아니다. **이 표가 기술하는 코드 기본값(off)은 devtools를 포함한 4개 패키지 전부에 아직 일괄 적용된다** — 패키지별로 게이트를 쪼개는 작업(devtools만 우선 on)은 harness#74에서 별도로 진행한다. |
