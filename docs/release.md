...
1. **skill 재설계** — `new-miniapp` 후처리와 `inject`(devtools facet)에서
   ...
   형태인지는 실증 시점의 CLI 산출물을 보고 확정한다. **`new-miniapp` 쪽은
   2026-08-10 결정으로 반영됐다**(아래 7번) — 다만 완전 삭제가 아니라 "확인 +
   실패 시 폴백"이다: `ait init` 실패를 CLI가 삼키고 exit 0으로 끝내는 경로가
   실재하므로(§7b 6번), 배선을 무조건 CLI에 맡기면 조용히 안 된 채로 넘어간다.
   `inject`(devtools facet)는 brownfield 전용이라 CLI가 개입하지 않는 자리이고,
   아직 손대지 않았다.
   import specifier는 `@apps-in-toss/devtools` 그대로다(subpath re-export
   경로로 바뀌지 않는다 — 재정의로 폐기된 전제다). `--no-devtools`는
   "설치 제외"에서 의미가 바뀐다 — 종착 형태는 아래 7번의 결정대로 "배선
   해제"다(CLI에 배선을 끄는 플래그가 없어 skip이 불가능하다).
...
7. **create-ait-app 버전 정책 + Step 4 정리 — 결정 완료(2026-08-10).**
   ...
   - **명시 핀 폐지, `@latest` 전환** — `create-ait-app`과 `@apps-in-toss/*`는
     무조건 최신을 쓴다. 핀(`@0.2.1`)이 upstream 개선 수신을 막는 쪽이 더
     컸다는 판단이다. 핀이 지탱하던 "산출물 형상이 결정적"이라는 전제는 skill의
     형상 가드가 매 run 확인하는 것으로 대체한다.
   - **Step 4는 확인 + 폴백으로 축소** — devtools 배선은 CLI(`ait init`)가
     하므로, skill은 devDependency와 번들러 설정의 unplugin이 실제로 배선됐는지
     확인하고 **안 돼 있을 때만** 기존 수동 배선을 수행한다.
   - **`--no-devtools`는 "배선 해제"로 재정의** — CLI가 이미 배선한 상태를
     전제로 devDependency 제거 + 설정에서 plugin 제거를 수행한다("설치 제외"는
     CLI에 그런 플래그가 없어 성립하지 않는다).

   전환 과정에서 확인된 사실 둘을 함께 기록한다(0.2.3 dist 실측, 2026-08-10).
   **`--skip-install`은 0.2.3에서 제거됐다** — 지정하면 `알 수 없는 옵션이에요:
   --skip-install`로 즉사하고 산출물이 0이라, 핀 상향은 곧 "scaffold와 install을
   분리하던 절차의 폐기"를 뜻한다. 그리고 **0.2.3은 `package.json`에
   `createAitApp` 메타데이터를 더 이상 쓰지 않는다**(`add-sample`은 그 필드가
   남아 있으면 오히려 제거하고, Apps in Toss 프로젝트 판정을
   `@apps-in-toss/web-framework` 의존성 또는 `apps-in-toss.config.ts` 존재로
   한다) — 그래서 그 필드를 보던 skill의 형상 가드는 0.2.3 산출물에서 항상
   실패했을 것이라, 이번 전환에서 판정 기준을 함께 교체했다.

   위 1번(skill 재설계)의 나머지(`inject` skill 정리 등)와 3·4번(eval fixture·
   baseline epoch)은 여전히 미착수다.
