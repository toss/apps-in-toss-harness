...
| `create-ait-app@0.1.3` 명시 핀 | `@latest` 사용 시 dist-tag 승격으로 조용히 깨지는 걸 막기 위한 고정 | **`@0.2.1`로 이관** — 제안 2(Release Notes/CHANGELOG 정비)가 수용되면 breaking change를 사전에 파악할 수 있어 다음 핀 승급 판단이 쉬워지지만, 이번 이관은 그 수용을 기다리지 않고 공개 registry 실측(latest=0.2.1)만으로 진행했다. **이후 폐기(2026-08-10)** — maintainer 결정으로 명시 핀 정책 자체를 버리고 `@latest`로 전환했다(형상 가드가 매 run 확인하는 것으로 대체, `docs/release.md` §7b 7번) |
