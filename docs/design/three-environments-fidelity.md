...
harness를 **fidelity 사다리 위의 사용자 대면 환경 겹**으로 재개념화한 설계 정본. 각 겹은 서로 다른 fidelity 천장을 가지며, 안쪽 겹이 구조적으로 못 하는 것을 바깥 겹이 메운다 — "측정됨 / 미구현 / 구조적 불가능"을 겹마다 명확히 구분한다. 변동하는 작업 상태는 GitHub Project가 source of truth고, 이 문서는 무시간 설계 근거를 담는다. grade 어휘는 `devtools/docs/mock-fidelity-catalog.md`와 동일: 🟢 faithful/measured · 🟡 partial/provisional · 🔴 inert/impossible/unverified.

작성 배경(2026-05-25): station별 도구는 정착했으나 (1) 환경별 fidelity 천장이 문서화되지 않았고, (2) relay attach에 인증이 없어 tunnel URL을 얻은 누구나 디버거를 붙일 수 있으며, (3) 로컬 fidelity가 약하다(safe-area는 iOS partner-portrait 하나만 실측-확정, 광고·하드웨어·no-op API는 호출이 관측조차 안 됨)는 세 문제가 동시에 드러났다. 이후(2026-05-27) PWA launcher를 fidelity 사다리의 독립 겹(환경 2)으로 명시하고 2026-06-03에 CDP relay를 양 끝 배선했으나, **2026-08-10 maintainer 결정으로 환경 2를 전면 제거**했다(harness#103) — 경위와 잔여 영향은 §0에 정리했다.

### 두 축은 직교한다 (fidelity ladder ⊥ Ring)

이 겹은 **fidelity 축** — "런타임이 얼마나 실제냐"(mock → 실 토스 WebView dev). GitHub Project의 동심원(Ring/M1~M5)은 **dev-cycle 호 축** — "사이클의 어디까지 덮느냐"(scaffold→dev→debug→ship→operate→plan→feedback). 두 축은 직교라 하나로 합치지 않는다. 대신 **각 환경 겹이 어느 마일스톤에서 acceptance를 거치는지**로 매핑한다(§1.4).

---

## 0. 이력 — 환경 2(PWA Sandbox launcher) 제거 (2026-08-10)

이 문서는 한때 **세 겹**(로컬 브라우저 · Sandbox App (PWA) · intoss-private relay dev)을 기술했다. 2026-08-10 maintainer 결정(harness#103)으로 가운데 겹인 **환경 2(PWA Sandbox launcher)를 코드·사이트·테스트·문서에서 전면 제거**했고, 사다리는 환경 1 ↔ 환경 3 **두 겹**으로 좁아졌다.

**번호는 재사용하지 않는다.** 남은 두 겹은 계속 "환경 1"·"환경 3"으로 부른다 — 이슈·커밋·로그가 이 번호에 걸려 있어 재번호는 대조 비용만 만든다. 환경 4(`relay-live`)도 앞서 같은 방식으로 제거됐다(#665).

제거된 표면:

- `sites/launcher/` — installable PWA shell 전체(GitHub Pages 배포 포함).
- `packages/debugger/src/dev-bridge/` — `debugger --mode=phone` CLI 래퍼와 `.ait_urls` writer.
- `relay-sandbox` mode 값(출력 env `relay-mobile`), `RelayOrigin`, 외부 relay 주소 발견 경로(`AIT_RELAY_BASE_URL`/`AIT_TUNNEL_BASE_URL`).
- launcher deep-link 합성(`LAUNCHER_URL`·`AIT_LAUNCHER_URL` override 포함).
- in-app gate의 페이지 호스트 allowlist에서 `*.trycloudflare.com` 분기.
- `setup-phone-preview` skill과 대응 command stub.

**환경 3은 온전히 남는다.** cloudflared quick tunnel(`packages/debugger/src/mcp/tunnel.ts`) · Chii relay · TOTP 시크릿 저장(`relay-secret-store.ts`)은 전부 환경 3이 소유한다. 환경 3의 quick tunnel이 나르는 것은 **CDP relay 소켓**이지 페이지 origin이 아니다 — 미니앱 페이지는 언제나 `*.private-apps.tossmini.com`에서 서빙된다. 그래서 페이지 호스트 allowlist에서 `*.trycloudflare.com`을 뺀 것이 환경 3 attach에 영향을 주지 않는다.

아래 §1~§8은 두 겹 기준으로 갱신돼 있다. 환경 2 시절의 설계 근거(devtools #377/#378/#379, agent-plugin #96/#104 등)는 git history에서 본다.

---

## 1. 환경 겹 × Fidelity 매트릭스
...
핵심 통찰: **fidelity 사다리는 안쪽 겹의 구조적 한계를 바깥 겹이 메우는 동심원이다.** 환경 1(mock)이 구조적으로 못 하는 실기기 엔진 거동·토스 WebView·SDK 네이티브 브리지를 환경 3(intoss-private dev)이 채운다 — 환경 3이 가장 바깥 겹(실 토스 WebView dev 런타임)이다. 바깥 겹일수록 실제에 가까워지되 진입 비용도 오른다(브라우저 즉시 → 토스 dog-food 번들 deploy). station 3(debug)과 6(operate)의 `logs`는 환경 3이 쓰는 같은 on-device CDP relay 인프라의 두 용도다(debug=개발 회귀 진단, operate=배포 후 런타임 관측).
...
| **실기기 엔진 거동** | 🔴 **구조적 불가** | desktop Chrome은 실기기 WebView 엔진이 아니다. CSS media query/touch/실 DPR/렌더 quirk 재현 불가 — **환경 3에 위임.** |
| host/UA-gated 코드 | 🔴 불가(브라우저 한정) | 브라우저는 `*.private-apps.tossmini.com` host를 못 만든다 — 환경 3에 위임. |

천장 한 문장: "계약 + 시각 레이아웃은 측정 가능하게 충실하다. 실기기 엔진 fidelity는 물리적으로 불가능하며, 그건 환경 3에 위임한다."
...
| 토스 WebView 런타임 + SDK 네이티브 브리지 | 🟢 측정 | 환경 1(mock)이 못 하던 실 토스 WebView·SDK 네이티브 브리지가 여기서 진짜로 돈다(`getOperationalEnvironment()="toss"`, swipe-back history-pop 관측). |
...
[환경 1] pnpm dev — desktop 브라우저 + mock SDK
   │ seam 1→3: RELEASE_CHANNEL=dogfood pnpm bundle:ait → ait deploy --scheme-only
...
- **seam 1→3의 비용은 번들 한 바퀴다**: 같은 dev 코드를 dogfood candidate로 빌드·업로드해야 실기기 겹에 오른다. 그래서 재현 가능한 회귀는 환경 1에서 먼저 좁히고, 엔진·SDK 브리지 fidelity가 걸린 것만 환경 3으로 올린다.
- `/ait debug`는 한 명령으로 환경을 자동 분기: ① relay attach 없음 + dev 서버만 → 환경 1 안내(브라우저 panel·`window.__ait`·DevTools), ② `intoss-private://` candidate → 환경 3 경로(QR/deep-link). (read-only skill은 관측 결과에 따라 분기하는 seam 규약.)
...
즉 환경 1·3이 M1(디버깅 가능한 dev 환경)의 fidelity 사다리를 함께 떠받친다.
...
(dev 모드 서버 `src/mcp/server.ts`는 별개 경로다 — HTTP mock-state endpoint 기반이고 attach 개념이 없어 이 2단계 모델의 대상이 아니다. 이 경로는 `pnpm dev`로 띄운 브라우저를 읽는 **환경 1**용 HTTP 면이고, 환경 1의 CDP 면은 §7.3 `LocalCdpConnection`(구현됨)이 `start_debug({mode:'local-browser'})`로 연다. 즉 이 절의 2단계 등록은 debug 모드(환경 3)에 적용된다.)
...
§5가 "plugin 안에서 도구가 언제 callable해지느냐"의 수명주기를 고정했다면, 이 절은 **하나의 MCP가 로컬 브라우저(환경 1)와 intoss-private 스킴(환경 3) 디버깅을 모두 담는** tool surface의 구조를 고정한다. 사용자 요구: "devtools MCP는 로컬 브라우저에서의 디버깅 및 HMR과 intoss-private custom scheme URL에서의 디버깅 모두를 지원해야 한다."
