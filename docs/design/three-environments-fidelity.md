...
| 영역 | grade | 내용 |
|---|---|---|
| installable PWA shell + 터널 진입 | 🟢 측정 | `e2e/fixture/launcher/`(`devtools.aitc.dev/launcher/` 배포, GitHub Pages). `pnpm dev:phone`(`AIT_TUNNEL=1`)이 cloudflared quick-tunnel + QR 출력 → 폰 홈 화면 PWA가 `?url=<tunnel>` deep-link로 dev 서버를 iframe 로드. `e2e/launcher.test.ts`로 검증. |
