...
|---|---|---|---|
| 1. GitHub Pages | launcher PWA + fixture 데모 사이트 | 쉽다 — Pages 비활성화 | **완료** (`https://toss.github.io/apps-in-toss-harness/`) — launcher 표면 자체는 이후 환경 2 제거(harness#103, 2026-08-10)로 삭제됐다 |
...
> **후속(2026-08-10)**: 이 Phase가 지킨 launcher 표면은 환경 2(PWA Sandbox
> launcher) 전면 제거(harness#103)로 사라졌다 — `sites/launcher/`, Pages 배포
> 워크플로, `LAUNCHER_URL`/`AIT_LAUNCHER_URL`이 모두 삭제됐다. 아래 체크리스트는
> 당시 실행 기록으로 보존한다. 경위는 `docs/design/three-environments-fidelity.md` §0.
