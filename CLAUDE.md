...
- `packages/agent-plugin/.claude-plugin/`이 플러그인 manifest — 타깃 아키텍처(기본: docs MCP + console MCP remote, opt-in: devtools devDependency + debugger MCP를 skill이 프로젝트 `.mcp.json`에 배선) 재작성은 #1에서 진행. **opt-in 축은 완료** — manifest에서 `mcpServers` 제거, `setup-debugger` skill이 프로젝트 `.mcp.json`에 배선. remote MCP 2종은 endpoint 가용(#3 GitBook·#4 MCP GW) 전까지 manifest에 넣지 않는다(placeholder 금지). 이관 트랙 전체는 milestone `MT — 공식 이관`(#1~#8).
