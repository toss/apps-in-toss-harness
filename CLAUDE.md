...
- **`check:dashboard-html-fresh`(`pnpm -r`)를 CI에서 지우지 마라.** `src/mcp/dashboard.generated.ts`는 커밋되는 빌드 산출물이라 `pnpm build`가 매번 덮어쓴다 — 손수정본이 커밋돼도 런타임엔 영향이 없어서 이 게이트가 없으면 조용히 어긋난 채 남는다. `--if-present`를 붙이지 않은 것도 의도다(두 패키지가 스크립트를 잃으면 pnpm이 실패하게 두어 게이트 사문화를 소리나게 한다).
