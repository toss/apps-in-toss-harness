...
이 저장소는 public이다 — 그럴수록 이슈·재현 자료에 시크릿·사내 식별자를 넣지 않는다. 이슈에 다음을 붙여넣지 않는다.
...
### 설치 계층 — 플러그인이 안 뜨거나 갱신이 안 될 때

`/ait:*`가 아예 안 뜨거나, 고친 게 세션에 반영이 안 될 때 이 증거가 필요하다.

붙일 것은 [설치 문제 해결](./install-troubleshooting.md)의 스냅샷 블록 출력이다. 블록은 그 문서 한 곳에만 두고 여기 다시 싣지 않는다.

스냅샷에는 `installPath`·`projectPath`·`installLocation`과 `marketplace list`의 Directory 소스에 홈 절대경로가 그대로 찍힌다 — 사용자명이 노출된다는 뜻이다. `sed "s|$HOME|~|g"`로 치환하고, 붙여넣기 전에 치환됐는지 눈으로 한 번 더 본다.

`plugin-catalog-cache.json` 전문은 붙이지 않는다 — 다른 플러그인 메타데이터까지 수백 KB가 들어 있다. 필요한 건 "키가 전부 `@claude-plugins-official`로 끝나는가" 한 줄뿐이다.
...
- [ ] 설치 상태 스냅샷을 붙였다면 경로의 사용자명을 `~`로 치환했다
