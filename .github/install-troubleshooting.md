...
| C5 | 플러그인은 enabled인데 skill 목록이 비어 보인다 | `installPath` 디렉터리는 있으나 `.claude-plugin/`이 없거나 `shared/skills/`가 비었다 | R4 → 안 되면 R5. 상류: anthropics/claude-code#64763 |
...
CLI는 새 세션을 연다. 데스크톱 앱은 창 닫기가 아니라 완전 종료 후 재실행이다 — 리로드 경로가 없다. C6에 쓰고, 다른 모든 단계 뒤의 마무리로도 쓴다.
...
[버그리포트 가이드](./bug-report-guide.md)의 "설치 계층" 절을 따라 스냅샷을 붙여 [이슈를 등록](https://github.com/toss/apps-in-toss-harness/issues/new/choose)한다. 이 문서의 명령은 Claude Code 2.1.269에서 확인했다.
