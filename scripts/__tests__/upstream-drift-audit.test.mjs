...
describe('filterCurrentPaths', () => {
  const paths = ['src/index.ts', 'dist/index.js', 'node_modules/.bin/x', 'README.md', 'src/shared/launcher-url.ts'];
  ...
  test('excludes localOnly — a harness-only file must never be reported as a deletion candidate', () => {
    // 실측 근거: src/shared/launcher-url.ts는 상류에 대응 파일이 없는 harness
    // 전용 신규 파일이라, localOnly로 보호하지 않으면 "삭제" 후보로 잘못 잡힌다.
    const out = filterCurrentPaths(paths, { localOnly: ['src/shared/launcher-url.ts'] });
    assert.equal(out.includes('src/shared/launcher-url.ts'), false);
  });

  test('keeps real source files', () => {
    const out = filterCurrentPaths(paths, { localOnly: [] });
    assert.deepEqual(out.sort(), ['README.md', 'src/index.ts', 'src/shared/launcher-url.ts'].sort());
  });
});
